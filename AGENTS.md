...
- The `pageBuilder` array (`sanity/schemaTypes/pageBuilderType.js`) sets `options.insertMenu` — group tabs plus a list/grid view toggle. Every block type must be listed in exactly one `groups[].of` array or it only shows under "All", so add new blocks to a group as well as to `of[]`. Grid tiles load `sanity/static/preview-<blockName>.png` (served at `/static/`, copied into `dist/` by `sanity build`); a missing image falls back to the block's `icon`, which is why each block schema uses a distinct `@sanity/icons` icon.
- `accountLoginBlock` and `applyEstimatorBlock` are intentionally **not** in `pageBuilder`'s `of[]` — they're built but unused, so they're hidden from the insert menu. Their schemas stay registered in `schemaTypes/index.js` and their cases stay in `src/components/page-builder.js`; re-enable by adding the two `defineArrayMember` lines back plus a group entry.
